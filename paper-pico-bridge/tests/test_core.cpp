...
int main() {
  ...
  std::array<Point,9> projective{}; for (int i=0;i<9;++i) { const float x=in[i].x, y=in[i].y, w=1.0f+0.08f*x+0.04f*y; projective[i]={(1.4f*x+0.2f*y+0.3f)/w,(0.1f*x+1.2f*y-0.2f)/w}; } Calibration projective_fit; assert(projective_fit.Fit(in, projective)); Point projective_extra=projective_fit.Map({1.25f,.35f}); const float extra_w=1.0f+.08f*1.25f+.04f*.35f; assert(std::fabs(projective_extra.x-(1.4f*1.25f+.2f*.35f+.3f)/extra_w)<1e-3f); assert(std::fabs(projective_extra.y-(.1f*1.25f+1.2f*.35f-.2f)/extra_w)<1e-3f);
  std::array<Point,9> pixel_in{}, pixel_out{}; for (int i=0;i<9;++i) { const float x=80.0f+120.0f*float(i%3), y=60.0f+60.0f*float(i/3), w=1.0f+0.0004f*x+0.0002f*y; pixel_in[i]={x,y}; pixel_out[i]={(0.003f*x+0.0004f*y+0.1f)/w,(0.0002f*x+0.0035f*y+0.05f)/w}; } Calibration pixel_fit; assert(pixel_fit.Fit(pixel_in,pixel_out)); Point pixel_extra=pixel_fit.Map({215.0f,135.0f}); const float pixel_w=1.0f+.0004f*215.0f+.0002f*135.0f; assert(std::fabs(pixel_extra.x-(.003f*215.0f+.0004f*135.0f+.1f)/pixel_w)<1e-3f); assert(std::fabs(pixel_extra.y-(.0002f*215.0f+.0035f*135.0f+.05f)/pixel_w)<1e-3f);
