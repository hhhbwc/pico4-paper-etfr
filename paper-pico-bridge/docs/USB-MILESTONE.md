...
The bridge dynamically discovers Paper USB devices by scanning sysfs and matching VID `0425`. It resolves the current bus node, CDC control/data interfaces, and bulk-IN endpoint at runtime, so it does not rely on a fixed `/dev/bus/usb` node or endpoint.
...
All USB capture is explicit and bounded. `--dual`, `--dual-record`, `--target-record`, and `--dual-live` accept only `1..30` seconds. A capture session claims the CDC interfaces, configures 115200 8N1 and DTR, reads bulk-IN data, then releases every claimed interface and closes its USB node before returning.
...
The Magisk service script creates a mode-`0700` runtime directory but deliberately does not start a persistent USB reader. Until a user-facing control protocol exists, all USB ownership remains in explicit bounded commands.
